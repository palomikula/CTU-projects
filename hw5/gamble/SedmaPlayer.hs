...
pickCard (card1:[]) s = tryToBeat card1 (myHand s) hist
-- | ((value card1) == 10) = tryToBeat card1 (myHand s) hist
...
pickCard (card1:card2:[]) s | (beats card2 card1) = tryToBeat card1 (myHand s) hist
...
--								  | (((beats card3 card1) || (not (beats card2 card1))) && (((value card1) == 10) || ((value card2) == 10) || ((value card3) == 10))) = tryToBeat card1 (myHand s) hist
								  | otherwise = tryToBeat card1 (myHand s) hist
								  where
									  match = tryToMatch card1 (myHand s)
