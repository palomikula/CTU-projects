...
_Bool pq_update(void *_pq, int label, int cost)
{
    ...
    if (
    	...
    	int crnt = pq->heapIDX[label];
		int prnt = GET_PARENT(crnt);
        while(GET_PARENT(crnt) >=0  && pq->cost[prnt] > pq->cost[crnt]){
            pq_swap(pq, pq->heapIDX[label], GET_PARENT(pq->heapIDX[label]));
			crnt = prnt;
			prnt = GET_PARENT(crnt);
        }
